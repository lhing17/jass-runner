...
| 核心特性 | 子项 | 状态 | 详细说明 |
| :--- | :--- | :--- | :--- |
| **语法解析** | 基础结构 | ⚠️ 部分 | 仅支持 `function` 定义、`local` 变量声明、`set` 赋值和函数调用。不支持 `globals` 块、`type` 定义、`native` 声明。 |
| | 复杂表达式 | ❌ 缺失 | 不支持任何复杂的数学或逻辑表达式解析（如 `(a + b) * c`）。 |
...
| | 全局作用域 | ❌ 缺失 | 无法解析或访问 `globals` 块中的变量。 |
| **标准库** | Native 函数 | ⚠️ 部分 | 仅通过 Python 模拟了极少量（<1%）的 `common.j` 函数。 |

## 2. 解释器模块 (Interpreter) 深度分析

### 2.1 AST 生成与遍历
- [ ] **完善 AST 节点结构**: `Parser` 目前仅能生成非常扁平的 AST。需要增加表达式树（Expression Tree）和控制流节点（IfStmt, LoopStmt 等）。

### 2.2 控制流语句 (Control Flow)
- [ ] **If/Else 语句**: 实现 `if / then / elseif / else / endif` 的解析与执行。
- [ ] **循环语句**: 实现 `loop / endloop` 的解析与执行。
- [ ] **退出语句**: 实现 `exitwhen` 的解析与执行。
- [ ] **返回语句**: 实现 `return` 的解析与执行（目前函数只能执行到末尾自然结束）。

### 2.3 运算符 (Operators)
- [ ] **算术运算符**: 实现 `+`, `-`, `*`, `/`。
- [ ] **比较运算符**: 实现 `==`, `!=`, `<`, `>`, `<=`, `>=`。
- [ ] **逻辑运算符**: 实现 `and`, `or`, `not`。
- [ ] **优先级与短路**: 实现正确的运算符优先级解析和逻辑短路求值。
...
### v0.2.0: 解释器核心补全 (Interpreter Core)
> 目标：使解释器能够运行纯算法类的 JASS 代码（如斐波那契数列计算）。

- [ ] **P0** [Interpreter] 实现控制流语句 (`if`, `loop`, `exitwhen`, `return`)。
- [ ] **P0** [Interpreter] 实现完整的运算符解析与表达式求值。
- [ ] **P0** [Interpreter] 实现 `globals` 全局变量块的解析与访问。
